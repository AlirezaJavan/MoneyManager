...
private fun String.persianIfNumeric(): String = if (all { it.isDigit() }) PersianNumber.toPersianDigits(this) else this
    ...
    if (showDeleteDialog) {
        AlertDialog(
            ...
            text = {
                Text(
                    stringResource(
                        R.string.feature_sms_impl_sms_rule_delete_message,
                        rule.bankName.ifBlank { rule.senderPattern.persianIfNumeric() },
                    ),
                )
            },
            confirmButton = {
