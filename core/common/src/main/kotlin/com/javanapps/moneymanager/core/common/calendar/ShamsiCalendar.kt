package com.javanapps.moneymanager.core.common.calendar
 ...
 *
 * The Gregorian <-> Jalali conversion uses the well-known integer "jalaali" algorithm (the same one
 * behind jalaali-js and Saman Zamani's PersianDate that the legacy app relied on), which is exact for
 * the supported range. All names are returned in Persian.
 *
 * This object is pure and framework-free (JVM `java.time` only) so it is trivially unit-testable.
 */
object ShamsiCalendar {
    ...
    fun weekdayName(date: ShamsiDate): String {
    ...
    fun fromGregorian(date: LocalDate): ShamsiDate {
    ...
    fun toGregorian(date: ShamsiDate): LocalDate {
    ...
    fun now(zone: ZoneId = ZoneId.systemDefault()): ShamsiDate {
    ...
    // The jalaali algorithm relies on truncation toward zero (JS `~~(a/b)`), not floor division —
    // they differ for negative operands such as `gm - 8`. Kotlin's `/` and `%` truncate, matching it.
    private fun div(
        a: Int,
        b: Int,
    ): Int = a / b
