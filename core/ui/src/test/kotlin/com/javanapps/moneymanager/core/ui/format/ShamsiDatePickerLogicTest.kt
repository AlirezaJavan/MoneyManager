...
/**
 * Tests for calendar logic driving ShamsiDatePickerDialog:
 *  - month lengths at every month / multiple years
 *  - day-clamping on month change
 *  - weekday labels (derived from toGregorian for consistency)
 *  - leap-year Esfand boundaries
 *  - all month names and weekday names
 */
class ShamsiDatePickerLogicTest {
    ...
    // ─── Weekday names (self-consistent: derived from toGregorian) ───────────
    ...
    @Test
    fun `weekday name of every first day of 1403 is consistent with toGregorian`() {
        for (month in 1..12) {
            val shamsi = ShamsiDate(1403, month, 1)
            val gregorian = ShamsiCalendar.toGregorian(shamsi)
            // ISO weekday: Mon=1..Sun=7; Persian week starts Saturday (Sat->0..Fri->6)
            val perIdx = (gregorian.dayOfWeek.value + 1) % 7
            val expected = ShamsiCalendar.WEEKDAY_NAMES[perIdx]
            val actual = ShamsiCalendar.weekdayName(shamsi)
            assertWithMessage("1403/$month/1 (Gregorian $gregorian)").that(actual).isEqualTo(expected)
        }
    }
    ...
    fun `known weekday dates are correct`() {
        ...
        assertThat(ShamsiCalendar.weekdayName(ShamsiDate(1403, 1, 1))).isEqualTo("چهارشنبه")
        // 1402-12-29 = 2024-03-19 = Tuesday (سه‌شنبه)
